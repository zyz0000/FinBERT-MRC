...
from utils.functions_utils import swa, load_model_and_parallel
...
def train(opt, model, train_dataset):
    ...
    model, device = load_model_and_parallel(opt, model)
    ...
    for epoch in range(opt.train_epochs):
        ...
        for step, batch_data in enumerate(train_loader):
            ...
            if global_step % save_steps == 0 and opt.local_rank == 0:
